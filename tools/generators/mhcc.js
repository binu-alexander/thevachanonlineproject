  ...
  Rev: 'RV' }
	...
	for (var i=startVerseIndex, il=endVerseIndex; i<il; i++) {
		...
		var
			...
			verseNumber1 = verseReferenceParts.length > 1 ? verseReferenceParts[1] : '',
			bookInfo = bibleData.getBookInfoByDbsCode(dbsBookCode);
			// console.log(verseNumber1);
			verseNumber2 = verseNumber1.split(',')[0];
			verseNumber = verseNumber2.split('-')[0];
		...
		*/
		...
		// console.log("verseNumber1   ",verseNumber1);
		currentChapter['html'] +=
				//bibleFormatter.openVerse(verseCode, verseNumber) +
				// Edited verseNumber into verseNumber1 by VIPIN
				'<span class="comm-v-num">Verse ' + verseNumber1 + '</span>' +
